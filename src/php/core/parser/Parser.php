...
class Parser
{
    ...
    * An element of or not element of operator is either ∈ or ∉, and a set is either a set literal, a set identifier.
        ...
        switch ($lookahead['type']) {
            ...
            case (Token::IDENTIFIER['name']):
                ...
                if(isset($rest['verticalline'])){
                    ...
                    if(!Functions::isVariablesGood(array_keys($setformula))) throw new SemanticException("At least one of the identifiers is misspelled in the formula.");
                    ...
                    if(!empty($wrog)) throw new UndefinedVariableException('The following variables are not defined: '.json_encode($wrog));
    ...
    * A divisibility operator can be either ∣ (divides) or ∤ (does not divide).
    * @return string|null The value of the divisibility operator, which is either "∣" or "∤", or null if the lookahead token is not valid.
    * @throws ParserException If the lookahead token does not match the expected token.
    */    
    ...
    * A logical operator can be either ∧ (AND) or ∨ (OR).
                ...
                if (isset($rest['op'])) {
                    switch ($rest['op']) {
                        case (Token::TOBEEQUAL['value']):
                            if (gettype($setoperationside) === 'string') {
                                if (isset($rest['set'])) {
                                    ...
                                    $this->vars->add($setoperationside, $rest['point']);
                                    $result = $rest['point'];
                                }
                            }
                            break;
                        ...
                        case (Token::DOT['value']):
                            if (gettype($setoperationside) === 'string') {
                                if ($this->vars->has($setoperationside)) {
                                    $set = $this->vars->get($setoperationside);
    ...
    private function stesruisc()
    {
        ...
        switch ($lookahead['type']) {
            ...
            case (Token::TOBEEQUAL['name']):
                ...
                $op = $lookahead['value'];
                $rest = $this->trhs();
                $result = array_merge_recursive(['op' => $op], [...$rest]);
                break;
            ...
            case (Token::LEFTPARENTHESIS['name']):
                ...
                $this->match(')');
                $rest2 = $this->stesruisc__();
                $array = array_merge_recursive(['lparen' => '(',], ['lhs' => $setoperationside], ['rest' => $rest], ['rparen' => ')'], ['rest2' => $rest2]);
                $result = Functions::evaluateSetExpression($array);
                break;
    ...
    */
    public function __construct($tokens = [])
    {
        $this->tokens = $tokens;
        $this->pos = 0;
        $this->vars =new Map(['A'=>new Set([1,2])]);
    }
    ...
    */
    ...
    private function getVarsByIds($ids){
        ...
        foreach ($ids as  $value) {
            $vars[$value]=$this->vars->get($value);
        }
        return $vars;
    }
